...
export class FilterPage implements OnInit, AfterViewInit {
  ...
  selectedItem(index, url, data) {
    ...
    if (index === 1) {
      ...
      this.router.navigate([
        `filter/${url}`,
        { label: this.filterPost.currentProcess },
      ]);
    }
  }
  ...
  setFilters() {
    ...
    this.filterPost.setLocations([]);
    this.filterPost.setBodies([]);
    this.filterPost.setCondition(fakeFeed[0]);
    this.filterPost.setModelYear(fakeFeed[1]);
    this.filterPost.setExteriorColor(fakeFeed[2]);
    this.filterPost.setInteriorColor(fakeFeed[3]);
    this.filterPost.setDoors(fakeFeed[4]);
    this.filterPost.setCylinderType(fakeFeed[5]);
    this.filterPost.setEngineSize(fakeFeed[6]);
    this.filterPost.setFuel(fakeFeed[7]);
    this.filterPost.setTransmission(fakeFeed[8]);
    this.filterPost.setDriveTrain(fakeFeed[9]);
    this.filterPost.setSeats(fakeFeed[10]);
    this.filterPost.setOrigin(fakeFeed[11]);
    this.filterPost.setInsurance(fakeFeed[12]);
    this.filterPost.setPlateType(fakeFeed[13]);
    this.filterPost.setDrivingReadiness(fakeFeed[14]);
    this.filterPost.setSaleType(fakeFeed[15]);
    this.filterPost.setWarrentyDuration(fakeFeed[16]);
    this.filterPost.setMakeModelTrims([]);
    
    // ADDING KEY VALUE FOR BADGE/COUNTER;
    // REMOVING EXTRA FILTER
    this.filterList = fakeFeed.filter((x) => {
      if (
    ...
    this.filterList.unshift(
      { name: { en: 'Body', ar: 'الشكل' }, path: 'Kbody' },
    ...
    this.filterPost.setFiltersList(this.filterList);
  }
}
